...
此模組負責「從影像中找出手，並轉成手勢辨識可以使用的數值資料」，主要任務：

1. 使用 MediaPipe Hands 偵測影像中的手部
2. 取得每一隻手 21 個關鍵點在畫面上的像素座標
3. 計算每一根手指的彎曲角度，判斷「伸直(1) / 彎曲(0)」
4. 回傳統一格式的資料給 `GestureRecognizer` 和 `web_app.py`

主要輸出資料格式：
-------------------
1. `find_position(...)`  → `landmark_list`
   - 內容：長度 21 的列表，每個元素為 `(id, x, y)`
   - 範例：`[(0, 320, 240), (1, 305, 235), ...]`
     - id：MediaPipe 關鍵點編號 0~20
     - x, y：在目前影像中的像素座標

2. `fingers_up(landmark_list)` → `fingers`
   - 內容：長度 5 的列表 `[thumb, index, middle, ring, pinky]`
   - 每個元素為 0 或 1：
     - 1：該手指被判定為「伸直」
     - 0：該手指被判定為「彎曲」
   - 範例：
     - `[0, 1, 1, 0, 0]` → 只伸出食指與中指（可當作「2」）
     - `[1, 0, 0, 0, 0]` → 只伸出大拇指（可當作「讚」）

3. `get_hand_count()` → int
   - 回傳目前畫面中偵測到的手數量（0, 1, 2）
   - 供 `web_app.py` 決定要處理幾隻手，並依「由左到右」排序組合結果

手部關鍵點編號（MediaPipe 定義）：
---------------------------------
- 點 0 : 手腕 (wrist)
...
---------------
- 透過「兩個向量的夾角」判斷手指彎曲程度
- 以手腕 → 關節 以及 關節 → 指尖 這兩個向量計算角度
- 若夾角 < 50° → 視為伸直（幾乎同一方向）
- 若夾角 ≥ 50° → 視為彎曲

此模組不直接決定「這是幾號手勢」，只負責把手指狀態整理成標準格式交給 `GestureRecognizer`。
"""
import cv2
